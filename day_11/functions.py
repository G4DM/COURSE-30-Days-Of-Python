...
def calculate_median(list_test):
    if len(list_test) & 2 == 0:
        even_median = round(0.5 * len(list_test))
        return even_median
    elif len(list_test) & 2 != 0:
        odd_median = round(0.5 * len(list_test) + 1)
        return odd_median
    
def calculate_mode(list_test):
    count = {}
...
def calculate_range(list_test):
    min_range = list_test[0]
    max_range = list_test[-1]
    formatted = f'The range of this list is from {min_range} to {max_range} ({min_range}-{max_range}).'
    return formatted

def calculate_variance(list_test):
    total_sum = 0
    for i in range(0, len(list_test), 1):
        total_sum += list_test[i]
    
    x_i = total_sum / len(list_test)

    variance = {}
    for i in range(0, len(list_test), 1):
        formula = (list_test[i] - x_i)**2
        print(formula)
        if list_test[i] not in variance:
            variance[list_test[i]] = formula

    formatted = f'The variance is: \n {variance}.'

    return formatted

def calculate_std(list_test):
    total_sum = 0
    for i in range(0, len(list_test), 1):
        total_sum += list_test[i]
    ...
    variance_sum = 0
    for i in range(0, len(list_test), 1):
        variance_sum += (list_test[i] - x_i)**2
    
    variance = variance_sum / len(list_test)

    std = variance ** 0.5

    formatted = f'The Standard Deviation is {std}.'
...
print(calculate_median(list_4))
print(calculate_mode(list_4))
print(calculate_range(list_4))
print(calculate_variance(list_4))
print(calculate_std(list_4))

# region Exercises Level 3

# Exercise 1: Write a function called is_prime, which checks if a number is prime.

def is_prime(number_to_check):
    prime = 0
    for i in range(number_to_check, 0, -1):
        division = number_to_check / i
        if division == 1 or number_to_check:
            prime += 1
...
# Exercise 2: Write a function which checks if all items are unique in the list

def is_unique(list_test):
    check_uniqueness = {}
    for i in range(0, len(list_test), 1):
        if list_test[i] in check_uniqueness:
            check_uniqueness[list_test[i]] += 1
        else:
            check_uniqueness[list_test[i]] = 1
    
    unique_values = list(check_uniqueness.values())

    for i in range(0, len(unique_values), 1):
        if unique_values[i] != 1:
            return "The list doesn't contain unique values."
    
    return "The list contains unique values"

list_unique = [1, 3, 5, 6, 7, 9]
list_not_unique = [1, 2, 3, 3, 5, 8, 8, 9]

print(is_unique(list_unique))
print(is_unique(list_not_unique))

# Exercise 3: Write a function which checks if all the items of the list are of the same data type.

def is_same_type(list_test):
    data_dict = {}

    for i in range(0, len(list_test), 1):
        if list_test[i] not in data_dict:
            data_dict[list_test[i]] = type(list_test[i])
    
    list_data = list(data_dict.values())
    first_value = list_data[0]
    check = 0

    for i in range(0, len(list_data), 1):
        if list_data[i] == first_value:
            check += 1
        else:
            check -=1
    
    if check == len(list_test):
        return "They have the same data types"
    else:
        return "They don't have the same data types"

data_list = [1, "John", False, 3]
same_data_list = ["John", "Percival", "Marcus"]

print(is_same_type(data_list))
print(is_same_type(same_data_list))

# Exercise 4: Write a function which check if provided variable is a valid python variable
...
def valid_variable(variable_name):
    ...
    converted_variable = list(str(variable_name))
    stringed_variable = str(variable_name)

    if converted_variable[0] in invalid_start or stringed_variable in invalid_start:
        return "The variable name is not valid."
    else:
        return "The variable name is valid."


first_variable = "_yes"
second_variable = "12novalid"
third_variable = False

print(valid_variable(first_variable))
print(valid_variable(second_variable))
print(valid_variable(third_variable))

# Exercise 5: Go to the data folder and access the countries-data.py file.
